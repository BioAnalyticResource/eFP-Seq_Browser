  ...
  if (rnaseq_calls.length == count_bam_entries_in_xml) {
    ...
    for (var i = 0; i < count_bam_entries_in_xml; i++) {
      if (bam_type_list[i] == "Google Drive") {
        ...
        data = { numberofreads: numberofreads_list[i], hexcodecolour: hexcode_list[i], gdrive: match_drive, filename: filename, tissue: rnaseq_calls[i][0], record: rnaseq_calls[i][1], locus: locus, variant: 1, start: locus_start, end: locus_end, yscale: yscale_input, status: status, struct: splice_variants};
        if (splice_variants == '') {
